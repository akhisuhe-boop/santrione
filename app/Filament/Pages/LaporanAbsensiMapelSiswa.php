...
class LaporanAbsensiMapelSiswa extends Page implements HasTable, HasForms
{
    ...
    public static function canAccess(): bool
    {
        return \App\Support\FeatureGate::tenantBolehLihatGrup(static::$navigationGroup)
            && auth()->user()->can('page_LaporanAbsensiMapelSiswa');
    }
            ...
            ->headerActions([
                \Filament\Tables\Actions\Action::make('export')
                    ...
                    ->action(function () {
                        ...
                        $riwayat = $query->orderBy('tanggal')->orderBy('siswa_id')->get();

                        return \Maatwebsite\Excel\Facades\Excel::download(
                            new \App\Exports\DetailAbsensiMapelExport($riwayat),
                            'Detail-Absensi-Mapel-Siswa-' . now()->format('Y-m-d') . '.xlsx'
                        );
                    }),
            ])
