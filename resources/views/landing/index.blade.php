...
<section id="harga" class="py-20 md:py-28 relative">
    <div class="mx-auto max-w-7xl px-4 lg:px-8 relative">
        ...
        @if($setting->promoSedangBerjalan())
            @php $hanyaCountdown = $setting->promo_hanya_countdown; @endphp
            <div id="promo-banner" class="mt-8 {{ $hanyaCountdown ? 'max-w-sm text-center' : 'max-w-2xl' }} mx-auto rounded-2xl bg-white border border-red-100 shadow-lg shadow-red-500/5 px-6 py-5 flex {{ $hanyaCountdown ? 'flex-col items-center' : 'flex-col sm:flex-row items-center justify-center sm:justify-between' }} gap-4">

                @if($hanyaCountdown)
                    @if($setting->promo_teks)
                    <p class="text-sm font-bold text-slate-800 flex items-center justify-center gap-1.5">
                        <i data-lucide="alarm-clock" class="w-4 h-4 text-red-500"></i>
                        {{ $setting->promo_teks }}
                    </p>
                    @endif
                @else
                <div class="flex items-center gap-3 text-center sm:text-left">
                    <span class="flex items-center justify-center w-10 h-10 rounded-xl bg-red-50 shrink-0">
                        <i data-lucide="flame" class="w-5 h-5 text-red-500"></i>
                    </span>
                    <div>
                        <p class="text-slate-900 font-bold text-sm leading-tight">{{ $setting->promo_teks }}</p>
                        <p class="text-red-500 text-xs font-bold">Hemat {{ $setting->promo_persen }}%</p>
                    </div>
                </div>
                @endif
